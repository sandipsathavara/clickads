 ...
 */
class sfValidatorMultiFile extends sfValidatorBase
{
   ...
   * @see sfValidatorBase
   */
  protected function doClean($images)
  {
    $arrFiles=array();
	...
	foreach($images as $k => $value ) 
	{
		...
		if( $value['name'] != '' && $value['tmp_name'] != ''  && $value['type'] != ''  && $value['size'] != ''   ) 
		{		
			...
			if ($this->hasOption('mime_types'))
			{
			 ...
			 $class = $this->getOption('validated_file_class');
				 ...
				 $arrFiles[] = new $class($value['name'], $mimeType, $value['tmp_name'], $value['size'], $this->getOption('path'));
